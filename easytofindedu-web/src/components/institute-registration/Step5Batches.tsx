...
interface Step5BatchesProps {
  ...
  onChange: (data: { batches: Batch[] }) => void;
  onNext: (data: { batches: Batch[] }) => void;
  onBack: () => void;
  onSaveDraft: (data: { batches: Batch[] }) => void;
  loading?: boolean;
}

const Step5Batches: React.FC<Step5BatchesProps> = ({ data, coursesData, onChange, onNext, onBack, onSaveDraft, loading }) => {
  const [batches, setBatches] = useState<Batch[]>(data?.batches || []);
  ...
  if (courses.length === 0) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-cream-100 mb-2">Batches & Schedule</h2>
          <p className="text-cream-100/60">Manage your batches and class schedules.</p>
        </div>

        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-6 flex items-start space-x-3">
          <AlertCircle className="w-6 h-6 text-yellow-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-medium text-cream-100">No Courses Available</p>
