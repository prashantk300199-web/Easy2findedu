...
function SocialSection({ social_media }: { social_media?: { youtube?: string; instagram?: string; facebook?: string } }) {
  const links = [
    social_media?.youtube ? { label: 'YouTube', href: social_media.youtube, icon: 'youtube' } : null,
    social_media?.instagram ? { label: 'Instagram', href: social_media.instagram, icon: 'instagram' } : null,
    social_media?.facebook ? { label: 'Facebook', href: social_media.facebook, icon: 'facebook' } : null,
  ].filter(Boolean) as { label: string; href: string; icon: string }[];

  if (!links.length) return null;

  return (
    <Sec id="social" title="Social Media & Online Presence">
      <div className="flex flex-wrap gap-4">
        {links.map(link => (
          <a
            ...
            className="flex items-center gap-3 rounded-xl border border-cream-300 bg-cream-50 px-6 py-4 text-sm font-semibold text-night-800 shadow-sm transition-all hover:border-gold-500 hover:bg-gold-50 hover:text-gold-700"
          >
            {link.icon === 'youtube' && (
      ...
      <Section className="py-10">
        <div className="grid gap-16 lg:grid-cols-[1fr_340px]">
          ...
          <div>
            ...
            <SocialSection social_media={hostel.social_media} />
